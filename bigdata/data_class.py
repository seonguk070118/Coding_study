...
#flann 변수 설정
    ...
    methods = [(sift, flannkdtree, 'flann_sift'),
        ...
        (brisk, flannLsh, 'flann_brisk'), (orb, flannLsh, 'flann_orb')]
    for (method, matcher, name) in methods:
        keyP1, des1 = method.detectAndCompute(img1, None) 
        keyP2, des2 = method. detectAndCompute(img2, None)
        mat = matcher.match(des1, des2)
        mat = sorted(mat, key=lambda x: x.distance)
        res = cv2.drawMatches(img1, keyP1, img2, keyP2, mat [0:20], None, flags=cv2.DrawMatchesFlags_NOT_DRAW_SINGLE_POINTS)
        cv2.imshow(name, res)
...
img1_src = cv2.imread("bigdata\images\img20.jpg", cv2.IMREAD_UNCHANGED) 
